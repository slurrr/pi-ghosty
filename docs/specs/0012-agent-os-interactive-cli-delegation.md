...
- **Spawn vs. Prompt:** Instead of calling `session.prompt` in-process, the coordinator uses `spawn` to trigger a `pi` CLI session.
...
- **Non-Blocking Orchestration:** The Coordinator remains active and responsive while workers are backgrounded.
- **Process Monitoring:** The Coordinator monitors the worker's process lifecycle. Upon a clean exit, it retrieves the generated `peer_report` from the filesystem and injects it back into the primary conversation.
- **Reliably Dumb Routing:** Sessions are reused based on Coordinator Session Affinity ("One Coordinator Session = One Worker Team") to ensure context consistency without over-engineering the routing logic.

### 4. Workflow Integration (CI for Ideas)
This infrastructure enables a multi-stage pipeline:
1. **Definition:** Seth and Coordinator riff to lock down a feature vision.
...
5. **Final Handoff:** The Coordinator pings Seth for user testing once the pipeline clears.
