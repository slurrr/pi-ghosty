...
to share tool load and task focus, while the user interacts only with a single main Corroborator.

## Scope
V1 includes:
- A Corroborator agent (the only agent the user talks to).
...
- A config file in the agent-tree root (`pi-agent.json`) defining:
  ...
  - nested agent objects (Corroborator + peers), each able to override: tool surface, skills, sampling params, etc.
...
- Persistent sessions for Corroborator + peers (so each agent can build working context), augmented by long-term memory.
...
- Corroborator delegates work to peers and receives concise results back (summary + artifacts + next actions).
- Peer sessions are context-isolated: only include the minimum task context and relevant artifacts, not full chat history.

### Memory (required)
- The Corroborator must have **active recall**: before responding, it retrieves relevant long-term memories and injects them
...
- Observation scoping:
 ...
 - `document_id` + tags strategy:
  - Retain one transcript document per agent session (Corroborator + each peer), each with a stable `document_id`.
...
- Each agent (Corroborator + peers) maintains a persistent session log (JSONL) so it can carry working context forward.
...
### Interfaces
- Local interface: PI TUI for interacting with the Corroborator.
- Remote interface (v1): Telegram messaging with the Corroborator (bot-based), **single-user only**.

### Logging & artifacts
- Write a per-session JSONL trace capturing:
  - user messages (redacted as needed)
  - corroborator/peer messages (or hashes + references if too large)
  - tool calls + results
...
- Tool permission gates can be implemented without Corroborator ad-hoc logic:
...
## Acceptance Criteria
- Running locally with vLLM at `http://localhost:8002`, the Corroborator can:
...
- Telegram details:
  ...
  - how to map the allowed chat ID to the single Corroborator session (persistent vs ephemeral)?
...
2. **Define config shape (`pi-agent.json`)**
   ...
   - Corroborator + peers (coder/researcher/reviewer) definition as nested objects.
...
4. **Corroborator orchestration loop**
   - Corroborator receives user messages, decides when to delegate, and spawns peer sub-sessions (same model endpoint).
   - Corroborator manages peer lifecycle and merges peer outputs into a user-facing response.
...
5. **Tool execution pipeline (config-gated)**
   ...
   - Tool calls are attributed to the active agent (Corroborator or specific peer).
   ...
   - Implement enforcement as a config-backed hook (e.g., via `beforeToolCall`/extension `tool_call` handler), not
     Corroborator ad-hoc logic.

6. **Interfaces**
   - Hook into PI TUI (local).
   - Implement Telegram gateway for the Corroborator (v1):
...
8. **Smoke test scenarios**
   - Corroborator-only conversation.
  ...
  - Memory: retain+recall loop works end-to-end (corroborator retrieves relevant memories on the next turn).
